...
export function InvoiceDetailScreen({ route, navigation }: any) {
  ...
  return (
    <Screen title={invoice.invoiceNumber}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}><Button mode="outlined" onPress={() => openOrSharePdf(invoice.pdfUrl, invoice.invoiceNumber)}>PDF</Button><Button mode="outlined" onPress={shareWhatsApp}>WhatsApp</Button><Button mode="outlined" onPress={() => { emailForm.reset({ email: invoice.customerSnapshot.email || '' }); setEmailOpen(true); }}>Email</Button></View>
      <AppCard><Text variant="titleMedium" style={{ fontWeight: '900', marginBottom: 8 }}>Payment status</Text><SegmentedButtons value={currentStatus} onValueChange={(value) => setLocalStatus(value as InvoiceStatus)} buttons={[{ value: 'pending', label: 'Pending' }, { value: 'paid', label: 'Paid' }, { value: 'cancelled', label: 'Cancelled' }]} /></AppCard>
      <View style={{ flexDirection: 'row', gap: 8 }}>{hasStatusChange && <Button mode="contained" loading={status.isPending} onPress={() => status.mutate(localStatus)} style={{ flex: 1 }}>Save invoice</Button>}<Button mode="contained" buttonColor={theme.colors.error} textColor={theme.colors.onError} onPress={() => setDeleting(true)} style={{ flex: 1 }}>Delete invoice</Button></View>
