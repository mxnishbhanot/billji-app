...
export const customerSchema = z.object({ name: z.string().trim().min(1, 'Customer name is required').max(120), phone: z.string().trim().min(1, 'Phone is required').max(24), email: z.union([z.literal(''), z.email('Enter a valid email')]).optional(), address: z.string().max(500).optional() });
...
export const settingsSchema = z.object({ businessName: z.string().trim().min(1, 'Business name is required').max(120), phone: z.string().max(24).optional(), gstNumber: z.string().max(32).optional(), email: z.union([z.literal(''), z.email('Enter a valid email')]).optional(), address: z.string().max(500).optional(), invoicePrefix: z.string().trim().min(1, 'Invoice prefix is required').max(12), theme: z.enum(['light', 'dark']), logoUrl: z.string().optional() });
